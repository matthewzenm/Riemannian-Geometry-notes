...
\begin{thm}
    ...
    Assume $\Ric{M}\geq(n-1)Kg$, then $\Delta d_p|_{\gamma(l)}\leq\Delta_kd_k|_{\gamma_K(l)}$.
\end{thm}
...
\begin{rem}
    \begin{enumerate}[(1)]
        ...
        For a proof, we refer to Schoen, Yau's \emph{Lectures on Differential Geometry}, Proposition 1.1.
    \end{enumerate}
\end{rem}
...
Then
\[\d\Vol_g=\sqrt{\det(g_{ij})}\d{r}\wedge\d{\theta^1}\wedge\cdots\wedge\d{theta^{n-1}},\]
