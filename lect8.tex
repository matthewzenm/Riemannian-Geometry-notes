...
Let $u:\Omega\subset\mathbb{R}^{n-1}\to\mathbb{R}$ be a function, then its graph $(x^1,\cdots,x^{n-1},u(x^1,\cdots,x^{n-1}))$ is a minimal surface if and only if it satisfies the following \textbf{minimal surface equation} (MSE):\index{minimal surface equation}
\[\div\left(\frac{\nabla u}{\sqrt{1+|\nabla u|^2}}\right)=0.\]
